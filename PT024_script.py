...
#os.chdir('/media/pteeuwen/7257-AEE5/Uni/Bachelor_Internship/Internship/Analysis_techniques')
...
m_Hstock1 = 1.36 * 10**(-3) # g  The weighed mass of the Host
V_solv_Hstock = 20.00  # mL The volume of the relevant volumetric flask
m_solv_Hstock1 = 22.69109 # g The weighed mass of the solvent + Host

# Host-stock 2
m_Hstock2 = 1.48 * 10**(-3) # g The weighed mass of the Host (Held at least 1h under vacuum)
m_solv_Hstock2 = 22.67117   # g The weighed mass of the solvent + Host

# Host-stock 3
m_Hstock3 = 1.44 * 10**(-3) # g The weighed mass of the Host
m_solv_Hstock3 = 22.71832   # g The weighed mass of the solvent + Host

# Guest-stock 1
M_Guest = 728.66 # g/mol   (S,S)-V2
m_Gstock1 = 1.48 * 10**(-3) # g The weighed mass of the Guest
V_solv_Gstock = 10.00 # mL +- 0.025 mL ; The volume of the relevant volumetric flask
m_solv_Gstock1 = 11.34373   # g The weighed mass of the solvent + Guest

# Guest-stock 2
m_Gstock2 = 1.18 * 10**(-3) # g The weighed mass of the Guest
m_solv_Gstock2 = 11.32977  # g The weighed mass of the solvent + Guest

# Guest-stock 3
m_Gstock3 = 1.60 * 10**(-3) # g The weighed mass of the Guest # Verander deze waarde, veranderd ook Ka van 1 en 2 ?????????/
m_solv_Gstock3 = 11.34258 # g The weighed mass of the solvent + Guest

# Host-measurement
V_solv_Hmeasure = 10.00 # mL +- 0.025 mL ;
m_Hstock_diluted1 = 0.35315 # g The weighed Host-stock before dilution, with known density calculated earlier
m_Hstock_diluted2 = 0.34290 # g
m_Hstock_diluted3 = 0.35310 # g
...
m_Gstock_diluted1 = 4.46687 # g The weighed Guest-stock before dilution, with known density calculated earlier
m_Gstock_diluted2 = 4.49703 # g
m_Gstock_diluted3 = 4.44804# g
m_Hstock_diluted_forGmeasurement1 = 0.17142 # g The weighed Host-stock before dilution that is added to the flask
m_Hstock_diluted_forGmeasurement2 = 0.17549 # g
m_Hstock_diluted_forGmeasurement3 = 0.17790 # g
# etc...

# Measurement
n = 3 #triplo
m1 = 34  #amount of measurements
m2 = 34
m3 = 34
#Each 35, check of stability. Same spectrum after 5 min?
...
for i in np.arange(1,n+1):
    ...
    m = 'm'+ str(i)
    vars()[name] = np.array([0,0,5,5,5,5,10,10,10,10,10,10,30,30,30,30])  # uL
    middlevalues = np.full(10,50)
    endvalues = np.array([100, 100, 100, 100, 200, 200, 250, 300])
    vars()[name] = np.concatenate(((vars()[name]), middlevalues))
...
def splitdata(fluorescence, n): #fluorescence is a string
    ...
    #os.chdir('/media/pteeuwen/7257-AEE5/Uni/Bachelor_Internship/Internship/Analysis_techniques/Fluorescence/' + Expnr + '/set' + str(n))
...
for i in np.arange(1,n+1):
    ...
    vars()[Gmolestitration] = vars()[volumetitration] * 10**(-3) * vars()[Guestconcentration] # Calculate total amount of moles of Guest in each measurement point, from the additions
    vars()[Hmolestitration] = vars()[volumetitration] * 10**(-3) * vars()[Hostconcentration] # Calculate total amount of moles of Host in each measurement point, from the additions
...
# 5) Fit data
    ...
    eps = 10**(-14) #Trick to keep H0 not as a variable and only optimize F0 and Ka ####Changed from -15, problem with bounds
    eps2 = 10**(-10)
...
print('{:.2e}'.format(np.average(bindingconstant[0])) + u" \u00B1 " + '{:0=3.2f}e+06'.format(np.std(bindingconstant[0]/10**6)))
print('The average Ka for peak 2 is:')
print('{:.2e}'.format(np.average(bindingconstant[1])) + u" \u00B1 " + '{:0=3.2f}e+06'.format(np.std(bindingconstant[1]/10**6)))
